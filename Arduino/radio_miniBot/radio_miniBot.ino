...
#define ServoMAX_H  180 // which may not be all the way from 0 to 180. 
#define ServoMIN_V  0  // Don't go to very end of servo travel
#define ServoMAX_V  180 // which may not be all the way from 0 to 180. 


/*-----( Declare objects )-----*/
SoftwareServo lservo;
SoftwareServo rservo;  // create servo objects to control servos
...
void setup()
{
  ...
  /*-----( Set up servos )-----*/
  lservo.attach(4);  // attaches the servo to the servo object
  rservo.attach(3);      // attaches the servo to the servo object
...
void loop()
{
 ...
 //Serial Print the values of joystick
  ...
  
  {
    ...
    VerticalServoPosition  = HorizontalJoystickReceived;
    HorizontalServoPosition  = VerticalJoystickReceived;
    ...
    if (HorizontalServoPosition >= 50 && VerticalServoPosition >= 90) {
      lservo.write(VerticalServoPosition+(HorizontalServoPosition-50));
      rservo.write(180-VerticalServoPosition);
    }
    if (HorizontalServoPosition >= 50 && VerticalServoPosition < 90) {
      lservo.write(VerticalServoPosition-(HorizontalServoPosition-50));
      rservo.write(180-VerticalServoPosition);
    ...
    if (HorizontalServoPosition < 50 && VerticalServoPosition >= 90) {
      lservo.write(VerticalServoPosition);
      rservo.write((180-VerticalServoPosition) - (50-HorizontalServoPosition));
    }
    if (HorizontalServoPosition < 50 && VerticalServoPosition < 90) {
      lservo.write(VerticalServoPosition);
      rservo.write((180-VerticalServoPosition) + (50-HorizontalServoPosition));
