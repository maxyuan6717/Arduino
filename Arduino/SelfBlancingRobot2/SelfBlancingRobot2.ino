...
float MOTORSLACK_B=40;                     // Compensate for motor slack range (low PWM values which result in no motor engagement)
...
bal_kp=map(analogRead(potP),0,1023,0,500);
bal_ki=map(analogRead(potI),0,1023,0,500);
bal_kd=map(analogRead(potD),0,1023,0,50);
rot_kp=ROTATION_KP;
rot_ki=ROTATION_KI;
rot_kd=ROTATION_KD;
...
void loop() 
{
  Serial.println(analogRead(potP));
  bal_kp=map(analogRead(potP),0,1023,0,500);
  bal_ki=map(analogRead(potI),0,1023,0,500);
  bal_kd=map(analogRead(potD),0,1023,0,50);
  rot_kp=ROTATION_KP;
...
void init_imu()
{
    ...
    mpu.setXAccelOffset(-5022);
    mpu.setYAccelOffset(1595);
    mpu.setZAccelOffset(1686);
  ...
  else 
{
  ...
  analogWrite(MOTOR_A_PWM,abs(MotorAspeed)+3);
